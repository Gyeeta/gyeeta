...
int GY_SSLCAP::attach_uprobes(pid_t pid, char (&errorbuf)[256])
{
	...
	if (libtype == SSL_LIB_OPENSSL) {
		...
		if (offset == 0) {
			snprintf(errorbuf, sizeof(errorbuf), "Failed to get SSL Lib Function SSL_free offset for PID %d", pid);
			return -1;
		}	
		...
		if (!plink) {
			snprintf(errorbuf, sizeof(errorbuf), "BPF : Failed to attach Function SSL_free for PID %d due to %s", pid, gy_get_perror().get());
			return -1;
		}	
