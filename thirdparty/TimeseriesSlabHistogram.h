...
#include "SlabHistogramBucket.h"
#include <string>

namespace folly {

/*
 * TimeseriesSlabHistogram tracks data distributions as they change over time.
 *
 ...
 * the min 99th percentile data value over the last 10 minutes?"
 *
...
template <
    ...
    class C = MultiLevelTimeSeries<T, CT>>
class TimeseriesSlabHistogram {
 private:
 ...
 public:
  ...
  ReturnType avg(size_t level) const {
    ...
    return detail::avgHelper<ReturnType>(total, nsamples);
  }
  ...
  ReturnType rate(size_t level) const {
    ...
    return detail::rateHelper<ReturnType, Duration, Duration>(
        total, elapsed);
  }
  ...
  size_t addValue(TimePoint now, const ValueType& value);
  /* Add a value the given number of times with timestamp 'now' */
  size_t addValue(TimePoint now, const ValueType& value, uint64_t times);
   ...
   * TimeseriesSlabHistogram once a second.
   */
  void addValues(TimePoint now, const Histogram<ValueType>& values);
  ...
  void addValues(Duration now, const Histogram<ValueType>& values) {
...
public :  
  detail::SlabHistogramBuckets<ValueType, ContainerType, GetIdxFromVal> buckets_;
