   ...
   creating a user namespace.
      ...
      First, we look at the run-time environment:
       ...
       Now start a new shell in new user (-U), mount (-m), and PID (-p) namespaces, with user ID (-M) and group ID (-G) 1000 mapped to 0 inside the user namespace:
           ...
           $ ./test_namespaces -p -m -U -M '0 1000 1' -G '0 1000 1' bash
...
usage(char *pname)
{
	...
	fprintf(stderr, "Example : ./test_namespaces -p -m -U -M '0 1001 1' -G '0 1001 1' /bin/bash\n\n");
...
main(int argc, char *argv[])
{
	...
	if (gid_map != NULL || map_zero) {
		proc_setgroups_write(child_pid, (char *)"deny");
