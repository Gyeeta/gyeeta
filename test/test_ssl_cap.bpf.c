...
static bool get_cleartext(struct ClearArgs *pcleararg)
{
...
static void ssl_ret_rw_probe(int rc, bool is_write)
{
...
{
	ssl_ret_rw_probe(rc, true /* is_write */);
}
...
{
	ssl_ret_rw_probe(rc, false /* is_write */);
}
...
static int upd_tcp_sock(struct sock *sk, bool is_write)
{
	u64 				pidtid = bpf_get_current_pid_tgid();
	struct tssl_tcp_val		*pval;
...
static int handle_ssl_shutdown(void *ssl, u64 pidtid, bool is_ssl_free)
{
	struct tssl_conn_info		*psslinfo;
	struct tssl_key			tkey = {};
	...
	gy_bpf_printk("%s : SSL ctx %p : Conn Map conn %p\n", is_ssl_free ? "SSL_free" : "SSL_shutdown", ssl, psslinfo);
...
{
	return handle_ssl_shutdown(ssl, bpf_get_current_pid_tgid(), false /* is_ssl_free */);
}

SEC("uprobe")
int BPF_UPROBE(ssl_free, void *ssl) 
{
	return handle_ssl_shutdown(ssl, bpf_get_current_pid_tgid(), true /* is_ssl_free */);
}
...
{
	return upd_tcp_sock(sk, true /* is_write */);
}
